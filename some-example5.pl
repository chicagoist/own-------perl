...
my $names_names = 'Fred Flinstone and Wilma Flinstone';
if( $names_names =~ m/(?<last_name>\w+) and \w+ \g{last_name}/ ) {
 say "I saw $+{last_name}";
}

if ("Hello there, neighbor Alice" =~ /\s(\w+),/) {
 ...
 #
 # Совпавший текст сохраняется в хеше с именем %+: ключом является
 # метка (то есть имя совпадения), а значением – совпавшая часть строки.
 # В данном случае имена состоят из знаков препинания: $&, $` и $'.
 # Странно, уродливо, непривычно, но это их имена. Часть строки,
